...
const (
	ResponseTimeout     = 600 // shardkv回复client的超时时间，单位：毫秒
...
func (kv *ShardKV) Get(args *GetArgs, reply *GetReply) {
	...
	case <-time.After(ResponseTimeout * time.Millisecond):
...
func (kv *ShardKV) executeClientCmd(op Op, commandIndex int, commandTerm int) {
	...
	if _, existCh := kv.notifyMapCh[commandIndex]; existCh {
		if currentTerm, isLeader := kv.rf.GetState(); isLeader && currentTerm == commandTerm {
...
func (kv *ShardKV) MigrateShard(args *MigrateArgs, reply *MigrateReply) {
	...
	kvMap := deepCopyMap(kv.kvDB[args.ShardNum]) // 只有双方config.Num相等时才将shard数据回复给请求者
