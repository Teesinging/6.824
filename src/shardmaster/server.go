...
func (sm *ShardMaster) Query(args *QueryArgs, reply *QueryReply) {
	...
	index, _, isLeader := sm.rf.Start(queryOp)
...
func shardLoadBalance(groups map[int][]string, lastShards [NShards]int) [NShards]int {
	// groups = {1: ["server1", "server2"], 2: ["server3"], 3: ["server4", "server5"], 4: ["server6"]} 这里其实我们只关注 1 2 3，具体里面有多少机器不关注
	...
	// 和下面的循环的区别在于，lastShards 中只有 123，而 groups 中有新加入的 4
	...
	for i := 0; i < groupNum; i++ {
		...
		curGid := gidSlice[i]
		delta := shardCnt[gidSlice[i]] - curTar // 该gid的组要变化的量
